(ns goodrx.main
  "Kotodama WASM entrypoint for the Goodrx clean-room actor (L4) — Clojure port.
  ...
  facts (`goodrx.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "goodrx")
(def tier "L4")
...
(def entity-specs
  [{:entity "Prescription" :plural "prescriptions" :id-prefix "goodrx_pre"
   ...
   {:entity "Claim" :plural "claims" :id-prefix "goodrx_cla"
   ...
   {:entity "Pharmacy" :plural "pharmacies" :id-prefix "goodrx_pha"
    :fields [:npi :name :network] :required [:npi :name]
    :coerce {} :refs {}}
   {:entity "Drug" :plural "drugs" :id-prefix "goodrx_dru"
    :fields [:ndc :name :strength :form] :required [:ndc :name]
    :coerce {} :refs {}}
   {:entity "Member" :plural "members" :id-prefix "goodrx_mem"
    :fields [:memberId :planId :groupId] :required []
    :coerce {} :refs {:memberId "Member"}}
   {:entity "PriorAuth" :plural "priorauths" :id-prefix "goodrx_pri"
...
(defn emit-facts
  "EAVT facts for one record: {\"goodrx.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "goodrx-compat" :tier tier :entities entities} 200])
...
(defn start! [] :goodrx-compat/ready)
